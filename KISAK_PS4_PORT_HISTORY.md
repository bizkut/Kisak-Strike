# Kisak-Strike PS4/OpenGNM Port History

This is the preserved implementation and hardware-test diary through the v4.94
checkpoint on 2026-07-16. It includes superseded plans and should be used as
evidence, not as the active work queue. See [KISAK_PS4_PORT.md](KISAK_PS4_PORT.md)
for current state, sequencing, and acceptance gates.
...
## v4.83-v4.94 continuation

This section archives the detailed candidate/result diary removed from the
active execution plan after the v4.94 hardware result. Its ordering is
preserved exactly so existing commit, hash, and attribution references remain
stable.

### v4.94 hardware result
...
The crash moves to the next default viewport panel. `CreatePanelByName("info")`
...
The next gate is the concrete `info`/text-window constructor after its last
mouse-input operation, then panel allocation and insertion return.
...
### v4.66: Trace the SourceScheme BaseSettings parser

Package version 3.32 and build marker `scheme_basesettings_trace_v466` identify
the targeted diagnostic for the v4.65 boundary. The PS4 `KeyValues` trace now
distinguishes SourceScheme input explicitly, brackets creation and error-context
setup for the root `BaseSettings` key, records its opening token, and marks entry
and return around its child recursion. A PS4-only bounded scope records at most
32 `section`, `key`, and `value` events with parser depth while that root is
being parsed. It is enabled immediately before the existing recursive call and
disabled immediately after it returns; tokenization, allocation, conditionals,
parser decisions, and non-PS4 behavior are unchanged.

The PS4 `tier1_client` and `engine_client` targets and the complete
toolchain-aware `kisak_ps4_monolithic` build complete successfully. The final
artifacts are:

```text
OELF: build-ps4-engine/kisak_ps4_monolithic
Size: 126,256,368 bytes
SHA-256: bd66570039b07c08d61977ea7b1e1323193e1f9d886f84e80381ee3985b906cf

SELF: build-ps4-engine/kisak_ps4_monolithic.bin
Size: 83,109,920 bytes
SHA-256: c6ab5028085bfce8118fa1aa48ece3908c1373f7a695e179745f88ffe44617df
...
`param.sfo` reports both `APP_VER` and `VERSION` as 3.32. The host PS4 suite
...
`panel named constructor complete` marker is the last log line, proving only
that the crash occurs after the `AnimationController` base class returns and
before the allocation expression returns to `CBaseModPanel`. It does not prove
that the visible constructor body has started.

OELF disassembly closes that distinction. After the inherited `Panel` call,
three members generated by `DECLARE_CLASS_SIMPLE` register the
`AnimationController` message, animation, and key-binding maps. Four
`CUtlVector` members are then constructed before the first body statement.
Only after those seven hidden/member stages do the scalar assignments,
`SetVisible(false)`, `SetProportional(true)`, and eight
`g_ScriptSymbols.AddString` calls run. The ordinary constructor walk already
proves that the translation unit initializer constructing `g_ScriptSymbols`
returns, so the current hardware evidence does not yet implicate that symbol
table.

### v4.78: Attribute AnimationController construction and the remaining base panel

Package version 3.44 and build marker
`animation_controller_trace_v478` identify the next manual-install hardware
test. This is an attribution build, not an inherited Xbox/PC compatibility
workaround. Skipping `new AnimationController(this)` would leave immediate
console-UI dereferences invalid and would only defer the same constructor to
the unconditional `vgui::GetAnimationController()` call in
`CBaseModPanel::RunFrame`.

PS4-only, class-filtered breadcrumbs now bound all three generated map
registrars. The message and key-binding dictionaries split lookup, pool
allocation, placement construction, `Construct`, and insertion. The animation
dictionary splits lookup, property-converter initialization, pool allocation,
and insertion. Each wrapper also records the child
`AnimationController` lookup and its chained `Panel` base lookup.

The PS4 constructor initializer list explicitly invokes the existing
`CUtlVector(int growSize, int initSize)` form with the same zero/zero defaults.
A semantics-preserving zero-return helper writes a marker before each of the four
vectors; the next marker proves the prior vector returned, and body entry
proves the last returned. The visible body then records every state,
visibility, proportional-layout, and common-symbol stage. Non-PS4 construction
keeps the original initializer unchanged.

The surrounding `CBaseModPanel` constructor records the allocation return,
animation-script result, console-settings result, menu/button/logo stages,
console dialog prefetch, and final labels. This lets the same hardware run
continue attributing a later failure if `AnimationController` clears. The
existing PC-derived `IsPC()` start-screen bypass is known to evaluate true on
PS4; it is deliberately observed but not repaired in this package. Once base
panel construction returns, a separate PS4 policy test will exclude PS4 from
that inherited bypass so the Legals → StartScreen → MainMenu sequence remains
independently attributable.

The narrow `vgui_controls_client`, `client_client`, and `engine_client`
targets compile, and the complete monolithic OELF/SELF link succeeds. The final
artifacts are:

```text
OELF: build-ps4-engine/kisak_ps4_monolithic.oelf
Size: 136,044,992 bytes
SHA-256: d39f1f20f472f022cee1a15b60884526b954f6b6e00a687634cb04eef7a528a9

SELF: build-ps4-engine/kisak_ps4_monolithic.bin
Size: 83,159,168 bytes
SHA-256: 9756adac327b65c99f3f93bf3e6247738848cfc9d7b1b032bcf7433a8a3999d6
...
The packaged eboot is byte-identical to the SELF. The OELF contains exactly one
v4.78 marker and one string for each top-level message-map, animation-map,
key-binding-map, constructor-body, and completed-base-panel probe; the v4.77
build marker is absent. Its symbol table contains one `g_ScriptSymbols`
object. Both packaging scripts pass `bash -n` and retain mode 0755.

PkgTool reports all 28 displayed limit, digest, and signature checks as
`[OK]`, with no displayed failure, and `param.sfo` reports both `APP_VER`
and `VERSION` as 3.44. The host PS4 suite remains at 11/14 with only the known
